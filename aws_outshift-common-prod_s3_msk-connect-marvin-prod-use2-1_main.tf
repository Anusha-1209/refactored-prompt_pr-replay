...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/outshift-common-prod/us-east-2/s3/msk-sink-connect-marvin-prod-use2-1.tfstate"
...
module "s3" {
  ...
  bucket_name = "msk-sink-connect-marvin-prod-use2-1" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
