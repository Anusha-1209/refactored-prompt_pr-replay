...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/outshift-common-prod/us-east-2/s3/outshift-platform-backstage-docs-prod.tfstate" # #note the path here. It should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
module "s3" {
  ...
  bucket_name           = "outshift-platform-backstage-docs-prod"
