...
terraform {
  backend "s3" {
    ...
    key    = "terraform-state/outshift-common-dev/eu-central-1/s3/outshift-zot-dev-bucket" # #note the path here. It should match the pattern terraform_state/<service>/<region>/<name>.tfstate
...
module "s3" {
  ...
  bucket_name           = "outshift-zot-dev-bucket"
