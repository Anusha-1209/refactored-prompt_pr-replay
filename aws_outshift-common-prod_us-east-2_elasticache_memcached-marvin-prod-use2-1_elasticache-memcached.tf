terraform {
  backend "s3" {
    ...
    key    = "terraform-state/outshift-common-prod/elasticache/us-east-2/memcached-marvin-prod-use2-1.tfstate"
    region = "us-east-2"
  }
}
...
locals {
  ...
  elasticache_name     = "marvin-prod-use2-1"
  memcached_vpc_name   = "marvin-prod-use2-data"
...
resource "aws_security_group" "memcached_security_group" {
  name = "memcached-marvin-prod-use2-1-sg"
