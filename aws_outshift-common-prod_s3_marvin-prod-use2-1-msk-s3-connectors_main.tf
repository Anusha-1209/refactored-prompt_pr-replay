...
terraform {
  backend "s3" {
    ...
    key = "terraform-state/aws/outshift-common-prod/us-east-2/s3/marvin-prod-use2-1-msk-s3-connect-plugins.tfstate"
...
module "s3" {
  ...
  bucket_name = "marvin-prod-use2-1-msk-s3-connectors" # The name of the S3 bucket.  S3 bucket names are required to be globally unique across all of AWS.
